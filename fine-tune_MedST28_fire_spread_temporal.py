...
from models.ViT.VisionTransformerModelTemporal import VisionTransformerConvTemporal, UNet3D
...
def fine_tune(
        ...
        input_image_size      = fine_tune_dataset[0][0].shape[2] # get input size by height of the image 64
        input_channels_number = fine_tune_dataset[0][0].shape[1] # get input channels 154
        input_time_steps_number = fine_tune_dataset[0][0].shape[0] # get input time steps 
        print(f"Input image size: {input_image_size}, Channels number: {input_channels_number}, Time Steps: {input_time_steps_number}")
        # print(fine_tune_dataset[0][0].shape)
        # exit()
        ...
        model = VisionTransformerConvTemporal(patch_size=patch_size,
       ...
       # Freeze all encoder parameters
        ...
        for epoch in range(num_of_epochs): # image shape = (batch, channels, height, width)
            ...
            for data in tqdm(fine_tune_data):
                ...
                variables, timestamps, label = data
                ...
                output = model(variables, timestamps)
                ...
                #fine_tune_metrics.update(output=outputs, label=label.unsqueeze(1))
