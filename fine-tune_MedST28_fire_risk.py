...
from utils.utils import get_years_months_fire_spread, get_files_FireRisk, string_to_boolean, create_fine_tune_plots
...
def fine_tune(
        ...
        data_files_pre_train = get_files_FireRisk(dataset_path, data_extension, fine_tune_years)

        # load the data file names (paths) for validation data
        data_files_validation = get_files_FireRisk(dataset_path, data_extension, validation_years)
        ...
        model = VisionTransformerLSTM(patch_size=patch_size,
       ...
       # Freeze all encoder parameters
        ...
        for epoch in range(num_of_epochs): # image shape = (batch, channels, height, width)
            ...
            for data in tqdm(fine_tune_data):
                ...
                variables, label, class_weights = data
                ...
                #loss_weighted = (loss * class_weights).mean()
                loss.backward()
                #loss.backward()
...
if __name__ == "__main__":
    ...
    with open('configs/MedST-28_configs/fine-tune_config_fire_risk.yaml', 'r') as file:
