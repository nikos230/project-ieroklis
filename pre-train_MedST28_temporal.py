...
import csv
import wandb
from utils.utils import get_years_months, get_files_MedST28, string_to_boolean
...
def pre_train(
        ...
        model = MaskedAutoEncoderViTModelTemporal(img_size=input_image_size,
        ...
        for epoch in range(num_of_epochs): # image shape = (batch, channels, height, width)
            ...
            for variables, timestamps in tqdm(pre_train_data):
                # pass input data to gpu device (if cuda is available)
                variables = variables.to(device)
                
                # pass the timestamps into gpu device (if cuda is available)
            ...
            with torch.no_grad():
                ...
                run.log({'pre-train loss': mean_loss_interation/len(pre_train_dataset), 'validation loss': mean_loss_interation_val/len(validation_data)})
...
if __name__ == "__main__":
    ...
    with open('configs/MedST-28_configs/pre-train_config_temporal.yaml', 'r') as file:
        pre_train_config = yaml.safe_load(file)
    file.close()

   # load dataset config
    with open('configs/MedST-28_configs/MedST28_dataset_temporal.yaml', 'r') as file:
